...
while True: # The code is running constantly, either because then pvpython does not have to boot-up again and its faster
    ...
    if simulation_requested==1: # When the UI sends a notification to this code to run.
        ...
        def generate_init(center,radius,number_of_particles,current_case,current_time): # Used to generate the intial files
            ...
            pd.DataFrame(positions).to_csv('./csv{0}/particles_positions.csv'.format(str(current_case)),index=None,compression=None) # Using parquet since it is way faster to write/read. Only issue is the need to compile your own version of paraview so the library pyarrow is included (venv)
            pd.DataFrame(positions).to_csv('./csv{0}/particles_positions_{1}.csv'.format(str(current_case),str(current_time)),index=None,compression=None)
...
data = pd.read_csv("./csv{0}/particles_positions.csv")
...
output.RowData.append(data.values[:,2], "Z")
        ...
        try:
            shutil.copyfile('./csv{1}/particles_positions_{0}.csv'.format(current_time,current_case),'./csv{0}/particles_positions.csv'.format(str(current_case))) # This copies the user displayed time as the current csv file, used for simulation when the user wants to continue the simulation that already have happenened
        except:
        ...
        for i in range(0,total_time,dt): # Create "fake" files that are mostly empty, not necessary but nice to have so that even if somehow paraview displays a case not yet generated, there will be no error message
            shutil.copyfile("csv{0}/particles_positions.csv".format(str(current_case)),"csv{1}/particles_positions_{0}.csv".format(i,current_case)) 
        ...
        while i < total_time: # The real time simulation mode will run until i reaches the specified time total_time. It is useful to have a long time so that the simulation never really stops.
         ...
         """
            ...
            pd.DataFrame(df).to_csv('./csv{0}/particles_positions.csv'.format(str(current_case)),index=None,compression=None) # Write the file to parquet so it can be read by paraview


            shutil.copyfile("csv{0}/particles_positions.csv".format(str(current_case)),"csv{1}/particles_positions_{0}.csv".format(i,current_case)) # A bit faster than rewritting all
            ...
            if p_wind_direction != wind_direction or p_injection_amount != injection_amount or p_injection_radius != injection_radius or p_injection_position != injection_position or p_coefficient_diffusion != coefficient_diffusion or p_wind_value != wind_value:
                ...
                afoam.CellArrayStatus = ['U']
                afoam.PointArrayStatus = ['U']
                shutil.copyfile("csv{1}/particles_positions_{0}.csv".format(i,current_case),"csv{0}/particles_positions.csv".format(str(current_case))) # Copy the requested time as the current time
            
            
                position=pd.read_csv("csv{0}/particles_positions.csv".format(str(current_case)))  # Now, we "came back" in time and ready to re-simulate the simulation with the new parameters, at the correct time
